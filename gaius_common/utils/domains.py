...
def _domain_name_validator():
    ...
    return RegexValidator(
        regex, message="Enter a valid Domain (Not a URL)", code="invalid_domain"
    )
...
class DomainChecker(ABC):
    ...
    def check(self, steps, aliases=[], continue_on_false=False):
        ...
        for step in steps:
            ...
            if checkrs is False and continue_on_false is False:
    ...
    def dns_cdn_challenge(self, needle, **_kwargs):
        ...
        if re.search(r'TXT\s+"%s"' % needle, res) is not None:
            return True

        return False

    def cname_query(self, **_kwargs):
        return all([len(dns.resolver.query(self.cname)) > 0])
    ...
    def source_cname(self, **_kwargs):
        ...
        res = "%s" % a.response
        if re.search(r"CNAME.+%s" % self.cname[0:10], res) is not None:
            return True
        elif (
            self.domain.count(".") == 1 and a.rdtype == dns.rdatatype.A
        ):  # Naked domain
            try:
    ...
    def source_visit_https(self, domain, cname, source=None):
        ...
        try:
            ...
            if tryhttps.returncode > 0:
                raise RuntimeError(
                    "Source https checked failed: %s" % (tryhttps.returncode,)
                )
    ...
    def source_visit(self, domain, source):
        ...
        try:
            ...
            r = requests.get(
                url, headers={"Host": "%s" % domain}, allow_redirects=False, timeout=2
            )
